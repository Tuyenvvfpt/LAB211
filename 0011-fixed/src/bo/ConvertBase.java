...
// Chuyển đổi từ hệ cơ số khác sang hệ cơ số thập phân
    public static String convertOtherToDecimal(int baseInput, String value) {
        ...
        BigDecimal fractionalResult = BigDecimal.ZERO; // Track fractional part
        if (parts.length > 1) {
            ...
            for (int i = 0; i < fractionalPart.length(); i++) {
                ...
                fractionalResult = fractionalResult.add(number);
                basePowerDec = basePowerDec.divide(new BigDecimal(baseInt),
                        fractionalPart.length() * 2, BigDecimal.ROUND_DOWN);
            }
        }

        result = result.add(fractionalResult); // Add fractional part if applicable
        ...
        if (fractionalResult.compareTo(BigDecimal.ZERO) == 0) {
            result = result.setScale(0, RoundingMode.DOWN); // Discard fractional part
        } else {
            result = result.setScale(8, RoundingMode.DOWN); // Keep fractional part
        }
